...
export default function PaymentPage() {
  ...
  useEffect(() => {
    ...
    if (!draft?.vehicleId || String(draft.vehicleId) !== String(vehicleId)) {
      navigate(`/rental-agreement?vehicle=${vehicleId}`);
      return;
    }
  ...
  const gatewayLabel = paymentGatewayAssets.find((gateway) => gateway.id === selectedGateway)?.label || "Gateway";
  ...
  return (
    <section className="payment-page payment-page-staged">
      <form className="payment-card billing-card" onSubmit={submit}>
        <header className="billing-hero">
          <h1>Billing details</h1>
          <p>Complete your payment and booking information.</p>
        </header>

        <div className="payment-step-strip">
          <article className="payment-step active">
            <strong>1</strong>
          ...
          <article className="payment-step">
            <strong>3</strong>
            <span>Confirmation</span>
          </article>
        </div>

        <section className="billing-section booking-info-card">
          <span className="billing-chip">Customer details</span>
        ...
        <section className="billing-form-grid">
          <label>
            <span>Business or contact name</span>
            <input
              value={customerDetails.businessName}
              onChange={(event) => setCustomerDetails({ ...customerDetails, businessName: event.target.value })}
          ...
          <label>
            ...
            <input
              ...
              onChange={(event) => setCustomerDetails({ ...customerDetails, contactEmail: event.target.value })}
          ...
          <label>
            ...
            <input
              ...
              onChange={(event) => setCustomerDetails({ ...customerDetails, contactPhone: event.target.value })}
              placeholder="Contact phone"
            />
          </label>
          <label>
            <span>Pickup date</span>
            <input value={draft.pickupDate} readOnly />
          </label>
        </section>
        ...
        <section className="payment-gateway-section">
          <div className="payment-gateway-heading">
            ...
            <p>Real gateway logos are stored in project assets. This screen still records a booking and payment form, not a live processor charge.</p>
          </div>
          <div className="payment-gateway-grid">
            {paymentGatewayAssets.map((gateway) => (
              <button
                key={gateway.id}
                className={`gateway-card ${selectedGateway === gateway.id ? "active" : ""}`}
                type="button"
        ...
        <section className="payment-detail-shell">
          <div className="payment-selected-gateway">Selected gateway: <strong>{gatewayLabel}</strong></div>

          {["mastercard", "visa", "american-express"].includes(selectedGateway) ? (
            <div className="billing-form-grid split-card-grid">
              <label>
                <span>Cardholder name</span>
                <input
                  value={gatewayDetails[selectedGateway].holder}
                  onChange={(event) => setGatewayDetails({
                    ...gatewayDetails,
                    [selectedGateway]: { ...gatewayDetails[selectedGateway], holder: event.target.value },
                  })}
              ...
              <label className="wide-field">
                ...
                <input
                  value={gatewayDetails[selectedGateway].number}
                  onChange={(event) => setGatewayDetails({
                    ...gatewayDetails,
                    [selectedGateway]: { ...gatewayDetails[selectedGateway], number: event.target.value },
                  })}
              ...
              <label>
                ...
                <input
                  value={gatewayDetails[selectedGateway].expiry}
                  onChange={(event) => setGatewayDetails({
                    ...gatewayDetails,
                    [selectedGateway]: { ...gatewayDetails[selectedGateway], expiry: event.target.value },
                  })}
                  placeholder="MM/YY"
                />
              </label>
              <label>
                <span>CVV</span>
                <input
                  value={gatewayDetails[selectedGateway].cvv}
                  onChange={(event) => setGatewayDetails({
                    ...gatewayDetails,
                    [selectedGateway]: { ...gatewayDetails[selectedGateway], cvv: event.target.value },
                  })}
                  placeholder="CVV"
                />
              </label>
          ...
          {selectedGateway === "paypal" ? (
            <div className="billing-form-grid split-card-grid">
              <label>
                ...
                <input
                  value={gatewayDetails.paypal.holder}
                  onChange={(event) => setGatewayDetails({
                    ...gatewayDetails,
                    paypal: { ...gatewayDetails.paypal, holder: event.target.value },
                  })}
              ...
              <label>
                ...
                <input
                  type="email"
                  value={gatewayDetails.paypal.email}
                  onChange={(event) => setGatewayDetails({
                    ...gatewayDetails,
                    paypal: { ...gatewayDetails.paypal, email: event.target.value },
                  })}
          ...
          {selectedGateway === "yas-mixx" ? (
            <div className="billing-form-grid split-card-grid">
              <label>
                <span>Mixx account name</span>
                <input
                  value={gatewayDetails["yas-mixx"].holder}
                  onChange={(event) => setGatewayDetails({
                    ...gatewayDetails,
                    "yas-mixx": { ...gatewayDetails["yas-mixx"], holder: event.target.value },
                  })}
                  placeholder="Account holder"
                />
              </label>
              <label>
                <span>Mixx phone</span>
                <input
                  value={gatewayDetails["yas-mixx"].phone}
                  onChange={(event) => setGatewayDetails({
                    ...gatewayDetails,
                    "yas-mixx": { ...gatewayDetails["yas-mixx"], phone: event.target.value },
                  })}
              ...
              <label className="wide-field">
                ...
                <input
                  value={gatewayDetails["yas-mixx"].reference}
                  onChange={(event) => setGatewayDetails({
                    ...gatewayDetails,
                    "yas-mixx": { ...gatewayDetails["yas-mixx"], reference: event.target.value },
                  })}
                  placeholder="Mixx transaction reference"
                />
              </label>
            </div>
          ) : null}

          <div className="payment-amount-table">
        ...
        <button className="solid-button payment-submit confirm-payment-button" type="submit" disabled={submitting}>
